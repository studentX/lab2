...
CN_BASE="admission_sample"
SVC_NAME="depadm.default.svc"
...
openssl req -new -key serverKey.pem -out server.csr -subj "/CN=${SVC_NAME}" -config server.conf
...
rm caKey.pem serverKey.pem serverCert.pem
rm *.csr
rm *.srl
rm *.conf